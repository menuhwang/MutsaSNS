package com.likelion.mutsasns.logging;
...
public class LogFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        log.info("{}| {} {}", REQUEST_LABEL, request.getMethod(), getRequestURI(request));
        stopWatch.start();
        filterChain.doFilter(request, response);
        stopWatch.stop();
        log.info("{}| {} {} Status:{} Time:{}ms", RESPONSE_LABEL, request.getMethod(), getRequestURI(request), response.getStatus(), stopWatch.getTotalTimeMillis());
    }
